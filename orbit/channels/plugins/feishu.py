...
class FeishuChannel(ChannelPlugin):
    ...
    async def _run_webhook_monitor(self, *, on_inbound: InboundHandler) -> None:
        ...
        app = FastAPI(title="Orbit Feishu Channel")
    ...
    async def _run_ws_monitor(self, *, on_inbound: InboundHandler) -> None:
        ...
        if self._feishu_cfg is not None:
            ...
            if not app_id or not app_secret:
                try:
                    from orbit.config import read_root_section
        ...
        if not app_id or not app_secret:
            raise RuntimeError(
                "Feishu WebSocket 模式需要配置 FEISHU_APP_ID 和 FEISHU_APP_SECRET（环境变量或 orbit configuration set-channels --channel feishu --app-id ... --app-secret ...）。"
            )
