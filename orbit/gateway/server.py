...
def create_app(
    *,
    ...
    # Load plugins (register tools from ORBIT_PLUGIN_DIR) before creating runner
    load_plugins()
    ...
    # Default: multi-agent stores under ~/.orbit/agents/<agentId>/sessions/sessions.json
    ...
    app = FastAPI(title="Orbit Gateway", version="0.1", lifespan=lifespan)
    ...
    async def handle_agent_stream_event(evt) -> None:
        # evt can be orbit.agents.events.stream.StreamEvent (our internal)
    ...
    async def rpc(body: Dict[str, Any]):
        ...
        if method == "agents.resolve_defaults":
            ...
            return {
                ...
                "payload": {
                    ...
                    "workspaceDir": f"~/.orbit/agents/{aid}",
                },
            }
        ...
        if method == "config.get":
            # Return full root config (~/.orbit/orbit.json) for dashboard inspection.
            try:
    ...
    # The SPA lives in orbit/dashboard/static and talks to:
