...
ENV_PLUGIN_ROOT = "ORBIT_PLUGIN_ROOT"
...
def _parse_plugin_dirs_from_env() -> List[Path]:
    """Return list of plugin directory paths from ORBIT_PLUGIN_DIR (colon or comma separated)."""
...
def discover_plugins(plugin_dirs: Optional[List[Path]] = None) -> List[PluginInfo]:
    ...
    If plugin_dirs is None: use ORBIT_PLUGIN_DIR env, then plugins.plugin_dirs from
    root config (~/orbit/orbit.json) if env is empty. Each path can be either
...
def _load_plugin_tools(plugin: PluginInfo, registry: Any) -> None:
    ...
    spec = importlib.util.spec_from_file_location(
        f"orbit_plugin_{plugin.name}_{tools_module_name}",
        module_file,
...
def load_plugins(
    ...
    Args:
        plugin_dirs: Directories to scan; if None, use ORBIT_PLUGIN_DIR.
