"""Register `orbit config` CLI commands for config read/write.

All config sections (llm, skills, channels, etc.) are stored by default in
~/.orbit/orbit.json. The `config read/write` commands operate on sections
of that file.
"""
...
def register_config_cli(program: click.Group, ctx: ProgramContext) -> None:
    ...
    @program.group(
        "config",
        help="Read and write config sections in ~/.orbit/orbit.json (llm, skills, channels, etc.)",
    )
    @click.pass_context
    ...
    def config_read(name: str, raw_output: bool) -> None:
        """Read a config section by NAME from ~/.orbit/orbit.json."""
    ...
    def config_write(name: str, input_path: Optional[Path], from_stdin: bool) -> None:
        """Write a config section by NAME into ~/.orbit/orbit.json.
        ...
        click.echo(f"Wrote config section '{name}' to ~/.orbit/orbit.json")
