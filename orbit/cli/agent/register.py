...
def register_agent_cli(program: click.Group, ctx: ProgramContext) -> None:
    ...
    @click.option("--agent-dir", default="", help="Override agent_dir (default: ~/.orbit/agents/<id>)")
    @click.option(
        "--workspace-dir",
        default="",
    ...
    @agent_group.command("list", help="List agents under ~/.orbit/agents")
    def agent_list() -> None:
        mgr = AgentManager()
        mgr.ensure_main()
    ...
    @agent_group.command(
        "set-llm",
        help="Set per-agent LLM overrides in agent.json (merged over global orbit.json llm)",
    )
    @click.argument("agent_id", nargs=1)
    ...
    @agent_group.command(
        "del",
        help="Delete an agent: remove ~/.orbit/agents/<agent-id>/ (sessions, workspace, agent.json)",
    )
    @click.argument("agent_id", nargs=1)
    @click.option(
        "--force",
    ...
    @click.option(
        ...
        type=int,
        default=None,
        help="agent.wait timeout (ms); omit for gateway default (see gateway.agentWaitTimeoutMs, env ORBIT_GATEWAY_AGENT_WAIT_TIMEOUT_MS)",
    )
