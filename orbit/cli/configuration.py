"""Configuration CLI for Orbit.

Configure LLM provider/model and channels (feishu, console) in ~/.orbit/orbit.json,
and optional per-agent LLM overrides in ~/.orbit/agents/<agentId>/agent.json.
...
def _run_agent_llm_config() -> None:
    """Interactive per-agent LLM overrides; writes ~/.orbit/agents/<agentId>/agent.json (llm key)."""
    mgr = AgentManager()
    mgr.ensure_main()
    ...
    click.echo("Global LLM (orbit.json), for reference:")
...
def _run_interactive_wizard() -> None:
    ...
    click.echo("Orbit configuration wizard")
    click.echo("")
...
def _auth_wizard_set() -> None:
    ...
    click.echo("")
    click.echo("Orbit tools auth wizard")
    click.echo("")
...
def register_configuration_cli(program: click.Group, _ctx) -> None:
    ...
    @program.group(
        name="configuration",
        help="Configure Orbit (LLM, channels, skills, etc.)",
    ...
    def set_llm(provider: str, model_id: str, base_url: Optional[str], api_key: Optional[str]) -> None:
        """Update LLM config and persist to ~/.orbit/orbit.json."""
        current = read_root_config()
    ...
    def set_channels(
        ...
        """Update channels config and persist to ~/.orbit/orbit.json."""
        current = read_root_config()
        kwargs = dict(
