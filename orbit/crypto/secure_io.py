...
- 提供 `get_default_encrypted_store()`，统一从 `ORBIT_SECRET_KEY` 读取密钥；
- 若环境未配置密钥，抛出明确错误。
"""
...
def is_encryption_enabled(env_var: str = "ORBIT_IS_ENC") -> bool:
...
def _load_key_from_env(env_var: str = "ORBIT_SECRET_KEY") -> bytes:
    ...
    if not raw and env_var == "ORBIT_SECRET_KEY":
        raw = os.getenv("MW4AGENT_SECRET_KEY", "").strip()
    if not raw:
...
def get_default_encrypted_store() -> EncryptedFileStore:
    ...
    Key 来源：`ORBIT_SECRET_KEY` 环境变量。
    """
    global _default_store
    if not is_encryption_enabled():
        raise EncryptionConfigError(
            "Encryption disabled via ORBIT_IS_ENC; falling back to plaintext."
        )
