...
class _ProviderSpec:
    ...
    default_model: str = ""
    api_key_env: str = "ORBIT_LLM_API_KEY"
...
def _load_llm_config() -> Dict[str, Any]:
    """Load LLM config from the default config store (~/.orbit/orbit.json, section \"llm\")."""
    try:
...
def _load_agent_llm_overrides(agent_id: Optional[str]) -> Dict[str, Any]:
    """Per-agent llm fragment from ~/orbit/agents/<agentId>/agent.json (key \"llm\")."""
    try:
...
def _resolve_llm_limits(params: AgentRunParams) -> Tuple[Optional[int], Optional[int]]:
    ...
    agent.json ``llm`` → global ``llm`` → environment (ORBIT_LLM_*).
    """
    g = _load_llm_config()
    ...
    context_window = _first_positive_int(
        ...
        os.getenv("ORBIT_LLM_CONTEXT_WINDOW"),
    )
    max_tokens = _first_positive_int(
        ...
        g.get("maxTokens"),
        g.get("max_tokens"),
        os.getenv("ORBIT_LLM_MAX_TOKENS"),
    )
...
def _resolve_llm_settings(
    ...
    params.* → agent.json ``llm`` → global ``llm`` → environment (ORBIT_LLM_*).
    """
    g = _load_llm_config()
    if not isinstance(g, dict):
        g = {}
    ...
    model = _first_non_empty_str(
        ...
        g.get("model"),
        g.get("model_id"),
        os.getenv("ORBIT_LLM_MODEL"),
    )

    base_url_s = _first_non_empty_str(
        a.get("base_url"),
        g.get("base_url"),
        os.getenv("ORBIT_LLM_BASE_URL"),
    )
    ...
    thinking_level = _normalize_thinking_level(
        _first_non_empty_str(
            ...
            g.get("thinkingLevel"),
            os.getenv("ORBIT_LLM_THINKING_LEVEL"),
        )
    )
