...
本文档描述在 **orbit Gateway `Orchestrator`** 上新增一种编排策略：在固定顺序的多 Agent 流水线（例如 **A → B → C**）每一轮末尾，由 **顶层监督 LLM** 根据当前产物（尤其 **C 本轮输出**）决定 **结束** 或 **再开一轮宏迭代**（反馈回到 A）。  
...
- 编排主逻辑：`orbit/gateway/orchestrator.py`（`router_llm` 每轮选人、`_task_dag` 无环执行）
- LLM 调用：`orbit/llm/backends.py` 中 `_call_openai_chat`
- 编排语义索引：`orbit/docs/orchestrating_shannon.md`（Shannon 策略分层）
- DAG 与环： `orbit/gateway/dag_spec.py`（**本方案不修改 DAG 环语义**）
