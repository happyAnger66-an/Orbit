...
本文基于姊妹仓库 **Shannon**（与 orbit 同级的 `Shannon/` 目录）中 **Go Orchestrator** 的源码与 `Shannon/docs/` 官方说明，梳理其「多工作流、DAG / ReAct / Research」等的**实现原理**。不涉及 Shannon 各语言服务的部署细节。
...
| 维度 | Shannon | orbit（当前 Gateway 编排） |
|------|---------|------------------------------|
