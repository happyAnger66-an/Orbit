...
Goal: migrate legacy single-file session store (e.g. ./orbit.sessions.json)
into the new per-agent store layout:
  ~/.orbit/agents/<agentId>/sessions/sessions.json (under ORBIT_STATE_DIR when set)
...
def auto_migrate_legacy_sessions(agent_manager: AgentManager, *, agent_id: str = "main") -> None:
    ...
    - ./orbit.sessions.json (common dev default)
    - <state-dir>/orbit.sessions.json (state-root legacy)
    """
    candidates = []
    try:
        candidates.append(os.path.abspath("orbit.sessions.json"))
    except Exception:
        pass
    try:
        state_root = Path(agent_manager.get("main").agent_dir).parent.parent
        candidates.append(str(state_root / "orbit.sessions.json"))
    except Exception:
        # Fall back to home-based guesses (new and legacy state dirs)
        try:
            from orbit.config.paths import get_state_dir

            candidates.append(str(Path(get_state_dir()) / "orbit.sessions.json"))
            candidates.append(str(Path.home() / ".orbit" / "orbit.sessions.json"))
        except Exception:
            pass
