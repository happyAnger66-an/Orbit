"""Multi-agent management for orbit.

Aligned to OpenClaw's multi-agent state layout, but kept intentionally small:
- Default agent: "main"
- Each agent has an agent_dir under ~/.orbit/agents/<agentId>
...
class AgentConfig:
    ...
    # Optional per-agent LLM overrides (merged with global ~/.orbit/orbit.json llm section).
...
class AgentManager:
    ...
    def delete(self, agent_id: str, *, allow_main: bool = False) -> bool:
        """Remove ``~/.orbit/agents/<agentId>/`` and all contents (sessions, workspace, etc.).
