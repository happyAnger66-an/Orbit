...
def resolve_global_skill_filter() -> Optional[List[str]]:
    """Global skills.filter from ~/.orbit/orbit.json (normalized).
...
def resolve_agent_skill_filter(agent_id: Optional[str]) -> Optional[List[str]]:
    """Per-agent skills allowlist from ~/.orbit/agents/<agentId>/agent.json.
...
def _resolve_home_skill_paths() -> List[Tuple[str, Path]]:
    """Skill dirs under home / state root (deduped).

    Uses ``<get_state_dir()>/skills`` first, then other common locations if they
    exist (deduped), including non-hidden ``~/orbit/skills`` and ``~/.mw4agent/skills``.
    """
    env_dir = os.environ.get("ORBIT_SKILLS_DIR", "").strip()
    if env_dir:
    ...
    candidates = [
        ...
        home / ".orbit" / "skills",
        home / "orbit" / "skills",
        home / ".mw4agent" / "skills",
    ]
    for p in candidates:
        resolved = p.resolve()
        if resolved.is_dir() and resolved not in seen:
            seen.add(resolved)
            out.append(("home", resolved))
    return out
...
def build_skill_snapshot(
    *,
    ...
    paths.extend(_resolve_home_skill_paths())
