...
def resolve_default_tool_timeout_ms() -> Optional[int]:
    """Read global default from env or ``tools`` section of ``orbit.json``.

    Supported keys (first non-empty wins): ``timeout_ms``, ``timeoutMs``, ``defaultTimeoutMs``,
    ``default_timeout_ms``.

    Env: ``ORBIT_TOOLS_TIMEOUT_MS`` (integer milliseconds, > 0).
    """
    env = os.environ.get("ORBIT_TOOLS_TIMEOUT_MS", "").strip()
    if env:
        try:
            v = int(env)
            if v > 0:
