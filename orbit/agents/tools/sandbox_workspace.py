...
def resolve_sandbox_sessions_root(cfg_manager) -> str:
    """Root directory for per-run sandbox working dirs (~/.orbit/sandbox-sessions by default)."""
    ...
    env = os.environ.get("ORBIT_SANDBOX_WORKSPACE_DIR") or os.environ.get(
        "ORBIT_SANDBOX_SESSIONS_DIR"
    )
