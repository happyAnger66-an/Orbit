...
This module provides a unified interface for reading/writing Orbit configuration files
...
class ConfigManager:
    """Manages Orbit configuration files with encryption support.

    Configuration files are stored in `~/.orbit/config/` by default.
    ...
    def __init__(self, config_dir: Optional[str] = None) -> None:
        ...
        Args:
            config_dir: Base directory for config files. Defaults to `~/.orbit/config/`.
        """
        if config_dir:
            self.config_dir = Path(config_dir)
        else:
            home = Path.home()
            self.config_dir = home / ".orbit" / "config"
...
def get_default_config_manager() -> ConfigManager:
    ...
    channels, etc.) of the single file ~/.orbit/orbit.json.
    """
