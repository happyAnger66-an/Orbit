"""End-to-end pytest test: Gateway + Agent + Channels complete flow.

This test verifies the complete flow:
1. Start Gateway
2. Channel receives inbound message
3. Channel dispatcher calls Gateway RPC
4. Gateway triggers AgentRunner
5. AgentRunner generates reply
6. Gateway returns reply via agent.wait
7. Channel delivers reply
"""

from __future__ import annotations

import json
import os
import socket
import subprocess
import sys
import time
import urllib.request
import uuid
from pathlib import Path

import pytest

from orbit.agents.runner.runner import AgentRunner
from orbit.agents.session.manager import SessionManager
from orbit.channels.dispatcher import ChannelDispatcher, ChannelRuntime
from orbit.channels.registry import ChannelRegistry
from orbit.channels.dock import ChannelDock
from orbit.channels.types import ChannelCapabilities, ChannelMeta, OutboundPayload
from orbit.channels.plugins.base import ChannelPlugin
from orbit.channels.types import InboundContext
from orbit.channels.plugins.console import ConsoleChannel


def _find_free_port() -> int:
    """Find a free port for testing."""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.bind(("127.0.0.1", 0))
        return int(s.getsockname()[1])


def _wait_for_health(base_url: str, deadline_s: float = 8.0) -> dict:
    """Wait for gateway to become healthy."""
    start = time.time()
    while time.time() - start < deadline_s:
        try:
            with urllib.request.urlopen(f"{base_url}/health", timeout=1.0) as resp:
                return json.loads(resp.read().decode("utf-8"))
        except Exception:
            time.sleep(0.15)
    raise RuntimeError("gateway did not become healthy in time")


def _rpc_call(base_url: str, method: str, params: dict, timeout_s: float = 5.0) -> dict:
    """Call Gateway RPC."""
    body = {"id": str(uuid.uuid4()), "method": method, "params": params}
    data = json.dumps(body).encode("utf-8")
    req = urllib.request.Request(
        url=f"{base_url.rstrip('/')}/rpc",
        data=data,
        method="POST",
        headers={"Content-Type": "application/json"},
    )
    with urllib.request.urlopen(req, timeout=timeout_s) as resp:
        return json.loads(resp.read().decode("utf-8"))


@pytest.fixture
def gateway_process(tmp_path: Path):
    """Start a gateway process for testing."""
    port = _find_free_port()
    base_url = f"http://127.0.0.1:{port}"
    session_file = str(tmp_path / "gateway.sessions.json")
    # Isolate gateway subprocess from developer machine config/state.
    state_dir = tmp_path / ".orbit"
    cfg_dir = tmp_path / "cfg"
    cfg_dir.mkdir(parents=True, exist_ok=True)
    (cfg_dir / "orbit.json").write_text(
        json.dumps({"llm": {"provider": "echo", "model_id": "echo"}}),
        encoding="utf-8",
    )

    cmd = [
        sys.executable,
        "-m",
        "orbit",
        "gateway",
        "run",
        "--bind",
        "127.0.0.1",
        "--port",
        str(port),
        "--session-file",
        session_file,
    ]

    proc = subprocess.Popen(
        cmd,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        text=True,
        env={
            **dict(os.environ),
            "ORBIT_STATE_DIR": str(state_dir),
            "ORBIT_CONFIG_DIR": str(cfg_dir),
        },
    )

    try:
        _wait_for_health(base_url)
        yield (proc, base_url, port)
    finally:
        proc.terminate()
        try:
            proc.wait(timeout=3)
        except subprocess.TimeoutExpired:
            proc.kill()


@pytest.fixture
def temp_session_file(tmp_path: Path) -> Path:
    """Create a temporary session file."""
    return tmp_path / "sessions.json"


@pytest.fixture
def session_manager(temp_session_file: Path) -> SessionManager:
    """Create a SessionManager with temporary file."""
    return SessionManager(str(temp_session_file))


@pytest.fixture
def agent_runner(session_manager: SessionManager) -> AgentRunner:
    """Create an AgentRunner."""
    return AgentRunner(session_manager)


@pytest.fixture
def channel_registry() -> ChannelRegistry:
    """Create a fresh channel registry (tests register their own plugins)."""
    return ChannelRegistry()


class _CapturePlugin(ChannelPlugin):
    def __init__(self, delivered: list[str]) -> None:
        caps = ChannelCapabilities(chat_types=("direct", "group"), native_commands=True, block_streaming=False)
        dock = ChannelDock(id="console", capabilities=caps, resolve_require_mention=lambda _acct: False)
        meta = ChannelMeta(id="console", label="Console(Test)", docs_path="/channels/console")
        super().__init__(id="console", meta=meta, capabilities=caps, dock=dock)
        self._delivered = delivered

    async def run_monitor(self, *, on_inbound):  # pragma: no cover
        raise RuntimeError("not implemented for tests")

    async def deliver(self, payload: OutboundPayload) -> None:
        self._delivered.append(payload.text)


@pytest.mark.asyncio
async def test_complete_flow_gateway_channels(
    gateway_process,
    session_manager: SessionManager,
    agent_runner: AgentRunner,
    channel_registry: ChannelRegistry,
) -> None:
    """Test complete flow: Channel -> Gateway -> Agent -> Reply."""
    proc, base_url, port = gateway_process

    # Create dispatcher with Gateway mode
    runtime = ChannelRuntime(
        session_manager=session_manager,
        agent_runner=agent_runner,
        gateway_base_url=base_url,
    )
    dispatcher = ChannelDispatcher(runtime=runtime, registry=channel_registry)

    # Create inbound context
    test_message = f"Test message {uuid.uuid4().hex[:8]}"
    ctx = InboundContext(
        channel="console",
        text=test_message,
        session_key="e2e:test",
        session_id="e2e-test",
        agent_id="e2e",
        chat_type="private",
        was_mentioned=True,
    )

    # Capture delivered messages
    delivered_messages: list[str] = []
    channel_registry.register_plugin(_CapturePlugin(delivered_messages))

    # Dispatch (should go through Gateway)
    await dispatcher.dispatch_inbound(ctx)

    # Verify message was delivered
    assert len(delivered_messages) > 0, "No messages were delivered"

    # Verify reply contains some content
    reply_text = " ".join(delivered_messages)
    assert len(reply_text) > 0, "Reply is empty"


@pytest.mark.asyncio
async def test_gateway_agent_wait_returns_reply_text(
    gateway_process,
    session_manager: SessionManager,
    agent_runner: AgentRunner,
) -> None:
    """Test that Gateway agent.wait returns replyText in payload."""
    proc, base_url, port = gateway_process

    # Call agent RPC
    idem_key = str(uuid.uuid4())
    agent_res = _rpc_call(
        base_url,
        "agent",
        {
            "message": "Hello, this is a test message",
            "sessionKey": "e2e:wait",
            "sessionId": "e2e-wait",
            "agentId": "e2e",
            "idempotencyKey": idem_key,
        },
        timeout_s=3.0,
    )

    assert agent_res.get("ok") is True, f"agent call failed: {agent_res}"
    run_id = agent_res.get("runId") or agent_res.get("payload", {}).get("runId")
    assert isinstance(run_id, str) and run_id, f"missing runId: {agent_res}"

    # Wait for completion
    wait_res = _rpc_call(
        base_url,
        "agent.wait",
        {"runId": run_id, "timeoutMs": 10000},
        timeout_s=12.0,
    )

    assert wait_res.get("ok") is True, f"agent.wait failed: {wait_res}"
    payload = wait_res.get("payload", {})
    assert payload.get("status") == "ok", f"expected ok status: {wait_res}"

    # Verify replyText is present
    reply_text = payload.get("replyText")
    assert reply_text is not None, "replyText should be present in payload"
    assert isinstance(reply_text, str), "replyText should be a string"
    assert len(reply_text) > 0, "replyText should not be empty"


@pytest.mark.asyncio
async def test_gateway_multiple_agent_calls(
    gateway_process,
    session_manager: SessionManager,
    agent_runner: AgentRunner,
) -> None:
    """Test multiple agent calls through Gateway."""
    proc, base_url, port = gateway_process

    # Make multiple agent calls
    results = []
    for i in range(3):
        idem_key = str(uuid.uuid4())
        agent_res = _rpc_call(
            base_url,
            "agent",
            {
                "message": f"Test message {i}",
                "sessionKey": f"e2e:multi:{i}",
                "sessionId": f"e2e-multi-{i}",
                "agentId": "e2e",
                "idempotencyKey": idem_key,
            },
            timeout_s=3.0,
        )

        assert agent_res.get("ok") is True
        run_id = agent_res.get("runId") or agent_res.get("payload", {}).get("runId")

        # Wait for completion
        wait_res = _rpc_call(
            base_url,
            "agent.wait",
            {"runId": run_id, "timeoutMs": 5000},
            timeout_s=7.0,
        )

        assert wait_res.get("ok") is True
        payload = wait_res.get("payload", {})
        assert payload.get("status") == "ok"
        results.append(payload.get("replyText"))

    # Verify all calls succeeded
    assert len(results) == 3
    assert all(r and len(r) > 0 for r in results)
