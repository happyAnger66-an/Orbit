...
from orbit.config.paths import (
    get_state_dir,
    resolve_agent_dir,
...
def test_custom_workspace_uses_orbit_memory_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv("ORBIT_STATE_DIR", str(tmp_path))
    monkeypatch.delenv("ORBIT_WORKSPACE_DIR", raising=False)
    custom = os.path.join(str(tmp_path), "myproj", "ws")
    os.makedirs(custom, exist_ok=True)
    db = resolve_memory_index_db_path("main", custom)
    assert db == str(Path(custom) / "orbit_memory" / "index.sqlite")
