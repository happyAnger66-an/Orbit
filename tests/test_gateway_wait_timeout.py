...
from orbit.gateway.wait_timeout import (
...
def test_resolve_default_two_hours(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.delenv("ORBIT_GATEWAY_AGENT_WAIT_TIMEOUT_MS", raising=False)
...
def test_resolve_from_config_and_env(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("ORBIT_GATEWAY_AGENT_WAIT_TIMEOUT_MS", "12345")
