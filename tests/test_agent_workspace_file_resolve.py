...
def test_resolve_agent_workspace_accepts_quoted_and_mixed_case_agents(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("ORBIT_STATE_DIR", str(tmp_path / "state"))
    mgr = AgentManager()
    aid = "q-agents"
    ws = Path(mgr.get_or_create(aid).workspace_dir)
    ws.mkdir(parents=True, exist_ok=True)
    ...
    assert rel == "AGENTS.md"
    assert Path(abs_path).name == "AGENTS.md"

    rel2, abs_path2 = _resolve_agent_workspace_file_abs(
        agent_manager=mgr,
        agent_id=aid,
        rel_path="AgEnTs.mD",
        prefer_existing_case_variant=True,
    )
    assert rel2 == "AGENTS.md"
