# orbit Sandbox 实施计划（对比 OpenClaw 与 Shannon）

本文对比 **workspace 内 OpenClaw 文档**（`docs/openclaw/sandbox.md`）所描述的沙箱与 **Shannon agent-core**（`docs/security/sandbox.md`）所描述的沙箱，并基于 orbit 现状给出 **分阶段、可落地的实施计划**。
...
| **orbit 已有近似** | `orbit/agents/tools/policy.py` 的 `ToolPolicyConfig` + `filter_tools_by_policy`（全局 profile/allow/deny，含 channel/user 覆盖）已类似 **通用工具策略**，但 **尚未**区分「普通 run」与「带 sandbox 标志的二次策略」。 | **无** gRPC SandboxService；仅有 per-agent `workspace_dir` 与部分 RPC 上的路径校验（如 `agents.workspace_file.*`）。 |

**结论**：要在 orbit 里谈「沙箱」，应 **同时规划两条线**——与 OpenClaw 对齐的 **Sandbox 工具策略（策略沙箱）**，以及可选的 **执行面隔离（执行沙箱，可借鉴 Shannon）**。二者可组合：策略沙箱缩小 LLM 可见工具；执行沙箱保证「即便工具被调用，也只能动会话目录 + 安全命令」。

---

## 2. 目标架构（orbit 内如何拼在一起）
...
- 产出：更新 `docs/manuals/configuration.md` 或 `orbit/docs/` 内简短说明 + 示例 JSON。
...
3. **命令类工具**  
   - 若 orbit 有 `exec`/`run_terminal_cmd` 等：  
     - **最小方案**：严格 allowlist + 无 shell + 超时 + cwd=会话目录（对齐 Shannon `SafeCommand` 思路，可用 Python 子进程实现）。  
     - **较重方案**：引入对 Shannon `SandboxService` 的 gRPC 客户端（复用 Shannon 部署），orbit 只做会话 ID 与策略编排。
...
- orbit 作为「编排与产品网关」，将 **session_id / user_id** 与 Shannon **对齐**，文件工具通过 `SandboxClient` 走 gRPC；**策略沙箱仍在 orbit 侧完成**，避免重复实现路径逻辑。
...
- orbit 当前工具策略：`orbit/agents/tools/policy.py`
