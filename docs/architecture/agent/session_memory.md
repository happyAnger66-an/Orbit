...
## Orbit vs OpenClaw：现状差异与待补能力（短期记忆）

本节用于对齐 OpenClaw 的 session 短期记忆链路，并记录 Orbit 当前实现的差异点与后续补齐清单（持续更新）。
...
- **Orbit（当前）**
  - Session 元数据：`orbit/agents/session/manager.py`（加密 JSON，字段较少）。
  - Transcript：`orbit/agents/session/transcript.py`（JSONL：header + 多行 `type=message`，每行保存一个 OpenAI 风格 message）。
...
OpenClaw 在一次 run 中会对历史做多层治理，Orbit 当前主要缺少：
...
- **leaf 回退 / branch 机制**：遇到崩溃/中断导致末尾 orphan user message，OpenClaw 通过 leafEntry.parentId 回退；Orbit 当前多为简单 drop。
- **compaction（历史压缩）**：OpenClaw 可将旧历史压缩为摘要继续增长；Orbit 当前暂无。
- **按 sessionKey 差异化 history limit**：OpenClaw 可按 DM/群聊不同限额；Orbit 当前以全局 limit 为主。

### 4）与 OpenClaw “sessions 纳入统一 memory search”相比的缺口

OpenClaw 会把 `"sessions"` 作为 memory sources 之一写入统一索引（embedding + FTS），因此 `memory_search` 可检索到近期会话内容。Orbit 当前：

- `orbit.memory.search` 主要检索 workspace 的 md（MEMORY.md、memory/*.md 等）。
...
## Orbit 当前已落地的 OpenClaw/Pi 风格能力（阶段性记录）

本节记录已在 Orbit 中实现的“Pi/OpenClaw transcript 关键机制”，用于后续持续对齐。

### 1）Transcript 扩展：entry 类型与链结构

文件：`orbit/agents/session/transcript.py`
...
文件：`orbit/agents/runner/runner.py`
