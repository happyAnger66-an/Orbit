...
Orbit 提供了一套统一的加密文件存储框架，用于保护所有敏感文件（配置、skills、sessions 等）。框架采用 AES-256-GCM 对称加密算法，提供机密性和完整性保护。
...
加密框架从环境变量 `ORBIT_SECRET_KEY` 读取密钥。
...
export ORBIT_SECRET_KEY="<生成的base64密钥>"
```
...
```python
from orbit.crypto import EncryptedFileStore, EncryptionConfigError
...
获取进程级别的默认加密存储实例（从 `ORBIT_SECRET_KEY` 读取密钥）：

```python
from orbit.crypto import get_default_encrypted_store
...
config_mgr.write_config("gateway", {
    "port": 18790,
    "bind": "127.0.0.1",
    "session_file": "~/orbit/gateway.sessions.json"
})
...
```python
from orbit.config import ConfigManager
...
```python
from orbit.skills import get_default_skill_manager
...
2. **文件权限**：
   - 确保配置目录权限：`chmod 700 ~/orbit`
   - 限制文件读取权限：`chmod 600 ~/orbit/config/*.json`
...
Orbit 的加密框架设计参考了 OpenClaw 的思路，但做了以下简化：
...
1. **密钥文件支持**：从 `~/orbit/secret.key` 读取密钥
...
- [Session 管理](../orbit/agents/session/manager.py)
