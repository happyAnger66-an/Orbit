# ChannelDock：含义与使用场景（对比 OpenClaw 与 Orbit）
...
## 2. Orbit 中的 ChannelDock（当前实现）

Orbit 的 `ChannelDock` 很精简，重点是：
...
- **群聊是否 require mention**（Orbit 已用；OpenClaw groups 也有）
...
在 Orbit 中，Dispatcher 目前支持两种执行路径：
