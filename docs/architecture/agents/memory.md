...
本文总结 OpenClaw 的**会话文件短期记忆**实现架构，以及 Orbit 中与之对齐的 **memory_tool**（Agent 工具）与 **memory-cli** 设计。完整 Memory 系统（向量索引、混合检索、MMR、时间衰减等）见 [docs/openclaw/memory.md](../../openclaw/memory.md)。
...
## 2. Orbit Phase 1：memory_tool 与 memory-cli

Orbit 已实现与 OpenClaw 对齐的 **memory_tool**（Agent 工具）和 **memory-cli**（CLI），当前为**仅文件、无向量**的 Phase 1。

### 2.1 组件一览

| 组件 | 说明 |
|------|------|
| **orbit.memory** | 模块：`list_memory_files(workspace_dir)`、`search(...)`、`read_file(...)`。来源为工作区根下：`AGENTS.md`、`SOUL.md`、`TOOLS.md`、`IDENTITY.md`、`USER.md`、`HEARTBEAT.md`、`BOOTSTRAP.md`、`MEMORY.md`、`memory.md` 以及 `memory/*.md`。检索为关键词匹配（无 embedding）。 |
...
- **默认工作区**：`~/orbit/workspace`（可通过环境变量 `ORBIT_WORKSPACE_DIR` 覆盖）。首次使用时自动创建。
- **Agent**：Runner 的 tool context 使用 `params.workspace_dir or get_default_workspace_dir()`，Gateway 发起 run 时传入该默认工作区。
- **CLI**：`memory status/search/get` 的 `--workspace` 默认为 `~/orbit/workspace`。
...
**可以。** Orbit 对 MEMORY.md、USER.md、IDENTITY.md 等 .md 文件的处理与 OpenClaw 兼容，可直接复用 OpenClaw 的 workspace 文件。
...
| **工作区路径** | OpenClaw 默认工作区为 `~/.openclaw/workspace`，Orbit 为 `~/orbit/workspace`。若要直接使用 OpenClaw 的同一批文件，可将 OpenClaw workspace 目录内容**复制或链接**到 `~/orbit/workspace`，或将 `ORBIT_WORKSPACE_DIR` 指向 OpenClaw 的 workspace 目录。 |

**小结**：同一套 MEMORY.md、USER.md、IDENTITY.md 等可直接在 OpenClaw 与 Orbit 间共用，仅需保证路径指向同一目录或拷贝到 Orbit 默认工作区即可。
...
- 实现位置：
  - `orbit/memory/`：list/search/read_file
  - `orbit/agents/tools/memory_tool.py`：memory_search、memory_get 工具
  - `orbit/cli/memory/register.py`：memory status/search/get 子命令
