# `web_search`：Orbit 实现与 OpenClaw 对比

本文梳理 `orbit` 当前 `web_search` tool 的实现方式，并对照 OpenClaw 的 `web_search` 设计，指出差异与仍缺失的能力点，作为后续对齐参考。
...
- 工具实现：`orbit/agents/tools/web_search_tool.py`
- 工具注册：`orbit/agents/tools/__init__.py`
...
Orbit 目前只有“边界块 + warning”，缺少 OpenClaw 的：
...
- **差异**：Orbit 将外部请求失败视为 `success=True` 的 error payload；OpenClaw 通常会在更高层统一错误分类（并结合 failover/重试/告警）。
