...
Orbit 参考 OpenClaw 的做法，将 **LLM provider / model 抽象为可配置项**，并且：
...
核心入口在 `orbit/llm/backends.py` 的 `generate_reply()`：
...
provider = (
    params.provider
    or os.getenv("ORBIT_LLM_PROVIDER")
    or cfg_provider
    or "echo"
).strip().lower()
...
由 `ConfigManager` 管理（见 `orbit/config/manager.py`）：

- 默认目录：`~/orbit/config/`
- 可通过环境变量覆盖：`ORBIT_CONFIG_DIR=/custom/path`
- 文件名：`llm.json`

该文件通过 `EncryptedFileStore` 使用 **AES-256-GCM** 加密（带魔术头 `ORBIT_ENC_v1`），密钥来自环境变量：

- `ORBIT_SECRET_KEY`：base64 编码的 32 字节随机数
...
1. **加密密钥**：
   - 只放在环境变量 `ORBIT_SECRET_KEY`，不要写入代码库或配置文件；
   - 使用强随机 32 字节密钥，并妥善保管。
...
3. **权限控制**：
   - 确保 `~/orbit/config` 目录的权限为 700，配置文件为 600；
   - 在多用户环境下避免共享同一配置目录。
