# Orbit Skills 实现差异分析与补齐计划

本文对比 `orbit` 当前 skills 实现与 OpenClaw 的 skills 体系，给出缺口清单与落地计划。

---

## 1. 当前 Orbit 已实现能力（基线）

基于代码现状，`orbit` 已具备以下基础能力：
...
当前 `orbit` 默认技能目录基本是单点（`~/orbit/skills` + plugin skills_dir），缺少：
...
当前 `orbit` 每次 run 都重新 `build_skill_snapshot()`，没有：
...
当前 `orbit` 快照结构仅有 `skills/count/prompt`，且挂载字段在 `metadata` 下，缺少：
...
当前 `orbit` 仅有通用 config 读写入口，没有专门 skills 运维命令，导致：
...
- 新增 CLI：
  - `orbit skills list`：列出生效技能、来源、描述；
  - `orbit skills check`：检查格式、冲突、缺失字段。
...
- 增加格式工具与写入模式：
  - `orbit skills export --format skill-md|json`
  - `orbit skills convert <name>`
...
- `orbit skills list/check` 可用于诊断；
- prompt 有基本截断与告警信息。

达到以上后，Orbit 的 skills 子系统将从“可运行”升级为“可运维、可扩展、可解释”。
