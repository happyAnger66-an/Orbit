...
本文介绍 `orbit` 命令行工具的基本用法，重点围绕当前已经实现的几个命令组：
...
- **环境变量**：`export ORBIT_PLUGIN_DIR=/path/to/plugin`（多个路径用 `:` 或 `,` 分隔）
- **配置文件**：在 `~/.orbit/orbit.json` 的 `plugins` 段中配置 `plugin_dirs`、可选 `plugins_enabled`
...
```bash
python -m orbit gateway status --url http://127.0.0.1:18790
```

或输出 JSON：

```bash
python -m orbit gateway status \
...
```bash
python -m orbit gateway call health \
  --url http://127.0.0.1:18790 \
  --params '{}' \
  --json
```
...
```bash
python -m orbit gateway call agent \
  --url http://127.0.0.1:18790 \
...
```bash
python -m orbit channels console run \
  --session-file orbit.sessions.json
```
...
python -m orbit channels telegram run \
  --session-file orbit.sessions.json
```
...
```bash
python -m orbit channels webhook run \
  --host 0.0.0.0 \
  --port 8080 \
  --path /webhook \
  --session-file orbit.sessions.json
...
为 **feishu-docs** 等 MCP 文档工具准备用户访问令牌（设备授权，落盘 `~/.orbit/feishu_oauth.json`）：

```bash
orbit feishu authorize
# 多账号：orbit feishu authorize --account <key>
# 状态：orbit feishu oauth-status
```
...
```bash
python -m orbit channels feishu run \
  --mode webhook \
  --host 0.0.0.0 \
  --port 8081 \
  --path /feishu/webhook \
...
所有配置项（llm、skills、channels 等）**默认统一存储**在单一文件 `~/.orbit/orbit.json` 中。`config` 子命令用于读写该文件中的**各个段（section）**。

### 5.1 默认配置文件

- **路径**：`~/.orbit/orbit.json`
- **结构**：顶层键为配置段名，例如 `llm`、`skills`、`channels` 等，每段为一个 JSON 对象。
- **加密**：若已配置加密（`ORBIT_SECRET_KEY` 等），整个文件会按加密框架存储；否则为明文 JSON。

### 5.2 读取配置段

```bash
orbit config read llm
```

- 从 `~/.orbit/orbit.json` 中读取 `llm` 段并输出；
- 若该段不存在则输出 `{}`。

输出单行原始 JSON：

```bash
orbit config read llm --raw
```
...
```bash
orbit config write llm --input llm.json
```

从 stdin 写入：

```bash
echo '{"provider":"openai","model_id":"gpt-4o-mini"}' | orbit config write llm --stdin
```

写入会**合并**到现有 `~/.orbit/orbit.json` 中：仅更新指定段，其它段保持不变。
...
- 路径：`~/.orbit/orbit.json`（llm、skills、channels 等均存于此文件）
- 内容示例：
...
配置会被写入到 `~/.orbit/orbit.json` 的 `llm` 段，并受加密框架控制（参考加密文档）。

### 6.3 查看当前根配置

```bash
orbit configuration show
```
...
`configuration auth` 子命令用于配置和检查 **tools 权限策略**（全局 / 按 channel / 按 user / 按 channel+user），底层写入 `~/.orbit/orbit.json` 中的 `tools` 段，与 Runner 中的 `ToolPolicyConfig` / `resolve_effective_policy_for_context` 保持一致。
...
```bash
orbit configuration auth wizard
```
...
```bash
orbit configuration auth show --json
```
...
```bash
orbit configuration auth effective \
  --channel feishu \
  --user-id ou_xxx \
  --owner \
  --authorized \
...
`dashboard` 命令用于快速打开基于浏览器的 **Orbit 控制台**，它通过：
...
```bash
orbit dashboard --url http://127.0.0.1:18790
```

或仅打印 URL，不自动打开浏览器：

```bash
orbit dashboard --no-open
```
...
- **Config** 标签：分区查看/编辑根配置 `orbit.json`。
