...
在 Orbit 中注册三个 Agent 工具，通过**飞书官方 MCP**（与 [feishu-openclaw-plugin](https://github.com/clawbot-plugins/feishu-openclaw-plugin) 中 `fetch-doc` / `create-doc` / `update-doc` 相同协议）读写云文档。

## 启用

1. 将**父目录** `plugins` 加入扫描路径（二选一）：
   - `export ORBIT_PLUGIN_DIR=/path/to/orbit/plugins`
   - 或在 `~/.orbit/orbit.json` 的 `plugins.plugin_dirs` 中写入该路径  
2. 启动 Gateway：`orbit gateway run`（会 `load_plugins()`）。

若使用 `plugins_enabled` 白名单，插件名为 **`feishu-docs`**（见 `plugin.json`）。

## 推荐：设备授权落盘（与 OpenClaw 体验接近）

1. 已在 `orbit.json`（或环境变量）配置好飞书应用的 **`app_id` / `app_secret`**（与通道机器人相同应用即可）。
...
3. 本机执行：
   ...
   ```
   按提示在浏览器完成授权后，**access_token / refresh_token** 会写入 **`~/.orbit/feishu_oauth.json`**（权限 `0600`）。`feishu-docs` 插件会自动读取并在过期前 **refresh**。
4. 多账号时：`channels.feishu.accounts` 下用 `--account <key>`；或用环境变量 **`FEISHU_OAUTH_APP_ID`** 指定要用哪套 `app_id` 的令牌。
5. 国际版 Lark：`orbit feishu authorize --brand lark`（或配置 `api_base` 含 `larksuite` 时插件侧会倾向 `lark` 端点）。
...
Orbit 已通过 **`orbit feishu authorize`** 与 **飞书会话内 `/mw4auth` 卡片** 提供用户授权入口；若均未配置，则需 **环境变量或配置明文 UAT**，否则工具会失败。
...
1. **策略**：默认 `tools.profile` 为 `coding` 时已包含 **`feishu_*`** glob，飞书文档三个工具会进入 LLM 的 tool 列表（见 `orbit/agents/tools/policy.py`）。若你改过 `tools.allow` / `by_channel` 等覆盖了 profile，请自行加上 `feishu_*` 或 `tools.profile: full`。
2. **禁用**：若不想暴露飞书工具，在 `tools.deny` 中加入 `feishu_*`。
3. **插件必须先被 Gateway 加载**：需配置 `ORBIT_PLUGIN_DIR` 或 `plugins.plugin_dirs`，否则 registry 里没有这些工具，模型仍看不到。
