...
name: feishu-docs
description: |
  Orbit feishu-docs plugin: feishu_fetch_doc, feishu_create_doc, feishu_update_doc.
...
1. Add parent directory `plugins` to `ORBIT_PLUGIN_DIR` or `plugins.plugin_dirs`.
2. User token: `/mw4auth` or `飞书授权` in Feishu chat, or `orbit feishu authorize`, or env `FEISHU_MCP_UAT` / config `channels.feishu.mcp_user_access_token`.
