...
Orbit的 **Next.js + Tauri** 桌面/Web 界面：左侧栏品牌 **Orbit**，**New Task / 新任务** 打开对话弹窗，**My Agents / 我的 Agents** 通过 `agents.list` 展示全部 Agent 并可一键用该 Agent 打开对话。协议：**HTTP `POST /rpc`** + **`WebSocket /ws`**。

## 功能（当前）

- 左侧栏：Orbit、New Task、My Agents；底栏网关地址与语言 / 主题
- 中英文切换（`localStorage`: `orbit-desktop-locale`）
- 浅色 / 深色主题（`next-themes`）
